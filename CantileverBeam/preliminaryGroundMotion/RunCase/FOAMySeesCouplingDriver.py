...
if __name__ == '__main__':# and rank==0:
	...
	# such that the array returned from openfoam can be used to initialize the coupling points for the coupling data projection mesh without writing to file...
	...
	if FOAMySees.config.oneWay==1:
		oneWay*=0
		onewaystatus="this is a one-way-coupled simulation: OpenSees Displacements are not transferred to OpenFOAM. The product of the OpenFOAM generated forces with the OpenFOAM surface displacements is zero."
	if FOAMySees.config.oneWay==2:
		oneWayD*=0
		onewaystatus="this is a one-way-coupled simulation, but work errors should not occur"
